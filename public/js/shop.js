...
window.triggerSearch = async function () {
    ...
    try {
        ...
        if (count) count.innerText = allSearchResults.length;
...
function renderPagination() {
    ...
    prevBtn.innerHTML = '←';
    prevBtn.className = `w-10 h-10 rounded-full flex items-center justify-center border font-bold text-xs transition ${currentPage === 1 ? 'border-gray-100 text-gray-300 cursor-not-allowed' : 'border-gray-300 text-black hover:bg-black hover:text-white hover:border-black'}`;
    prevBtn.disabled = currentPage === 1;
    prevBtn.onclick = () => { if (currentPage > 1) { currentPage--; renderProducts(); window.scrollTo({ top: document.getElementById('shop-section').offsetTop - 100, behavior: 'smooth' }); } };
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        btn.innerText = i;
        if (i === currentPage) {
            btn.className = 'w-10 h-10 rounded-full flex items-center justify-center bg-black text-white font-bold text-xs shadow-lg';
        } else {
            btn.className = 'w-10 h-10 rounded-full flex items-center justify-center border border-gray-200 text-gray-500 font-bold text-xs hover:border-black hover:text-black transition';
            btn.onclick = () => { currentPage = i; renderProducts(); window.scrollTo({ top: document.getElementById('shop-section').offsetTop - 100, behavior: 'smooth' }); };
        }
    ...
    nextBtn.innerHTML = '→';
    nextBtn.className = `w-10 h-10 rounded-full flex items-center justify-center border font-bold text-xs transition ${currentPage === totalPages ? 'border-gray-100 text-gray-300 cursor-not-allowed' : 'border-gray-300 text-black hover:bg-black hover:text-white hover:border-black'}`;
    nextBtn.disabled = currentPage === totalPages;
    nextBtn.onclick = () => { if (currentPage < totalPages) { currentPage++; renderProducts(); window.scrollTo({ top: document.getElementById('shop-section').offsetTop - 100, behavior: 'smooth' }); } };
    container.appendChild(nextBtn);
}


// Shared addToCart (kept for compatibility if needed elsewhere)
