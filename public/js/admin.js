...
async function loadOrders() {
    try {
        ...
        if (!orders || orders.length === 0) {
            if (container) container.innerHTML = `<tr><td colspan="6" class="p-8 text-center text-gray-400 font-medium">No orders found.</td></tr>`;
            if (recentContainer) recentContainer.innerHTML = `<tr><td colspan="4" class="p-6 text-center text-gray-400 text-xs">No recent orders.</td></tr>`;
            return;
        }

        // Populate Main Order List
        if (container) {
        ...
        // Populate Dashboard Recent Orders (Top 5)
        if (recentContainer) {
            recentContainer.innerHTML = orders.slice(0, 5).map(o => `
                <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition">
                        <td class="p-4 font-mono text-[10px] font-bold text-gray-500">#${o._id.slice(-6).toUpperCase()}</td>
                        <td class="p-4">
                        <div class="font-bold text-xs text-gray-800">${o.customerName}</div>
                        </td>
                        <td class="p-4 font-bold text-xs text-gray-800">${money(o.totalPrice)}</td>
                        <td class="p-4">
                        <span class="inline-block px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${o.status === 'paid' ? 'bg-green-100 text-green-700' : o.status === 'shipped' ? 'bg-blue-100 text-blue-700' : o.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}">
                            ${o.status}
                        </span>
                        </td>
                </tr>
